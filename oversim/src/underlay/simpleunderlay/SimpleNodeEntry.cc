...
//            tempTx->par(i).acceptDefault();
    ...
    rx.bandwidth = 30000000.0L*8.0L; //tempRx->par("datarate");
    rx.errorRate = 0; //tempRx->par("ber");
    rx.accessDelay = 0; //tempRx->par("delay");
    rx.maxQueueTime = (sendQueueLength * 8.0) / rx.bandwidth;
    rx.finished = 0;

    tx.bandwidth = 30000000.0L*8.0L; //tempTx->par("datarate");
    tx.errorRate = 0; //tempTx->par("ber");
    tx.accessDelay = 0; //tempTx->par("delay");
