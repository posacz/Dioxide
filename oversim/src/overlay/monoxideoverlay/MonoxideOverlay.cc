...
/**
 * @file MonoxideOverlay.cc
 * @author Sebastian Mies, Ingmar Baumgart, Bernhard Heep
 */

#include "overlay/monoxideoverlay/MonoxideOverlayMessage_m.h"

#include <assert.h>
#include <algorithm>

#include "../monoxideoverlay/MonoxideOverlay.h"
...
Define_Module(MonoxideOverlay);

std::ostream& operator<<(std::ostream& os, const MonoxideOverlayBucket* n)
{
    if (n == NULL)
        os << "NULL";
    else {
        for (MonoxideOverlayBucket::const_iterator i=n->begin(); i !=n->end(); i++) {
            os << *i << endl;
        }
...
class MonoxideOverlayLookupListener : public LookupListener
{
private:
    MonoxideOverlay* overlay;
public:
    MonoxideOverlayLookupListener(MonoxideOverlay* overlay)
    {
...
void MonoxideOverlay::initializeOverlay(int stage)
{
    if (stage != MIN_STAGE_OVERLAY)
        return;

    // MonoxideOverlay provides KBR services
    kbr = true;
    ...
    siblingTable = new MonoxideOverlayBucket(s * 5, NULL);

    // initialize pointers
    routingTable.assign(numBuckets, (MonoxideOverlayBucket*)NULL);
...
MonoxideOverlay::MonoxideOverlay()
{
    siblingTable = NULL;
    comparator = NULL;
...
void MonoxideOverlay::finishOverlay()
{
    simtime_t time = globalStatistics->calcMeasuredLifetime(creationTime);
    if (time < GlobalStatistics::MIN_MEASURED) return;

    globalStatistics->addStdDev("MonoxideOverlay: Nodes replaced in buckets/s",
                                nodesReplaced / time);
    globalStatistics->addStdDev("MonoxideOverlay: Bucket Refreshes/s",
                                bucketRefreshCount / time);
    globalStatistics->addStdDev("MonoxideOverlay: Sibling Table Refreshes/s",
                                siblingTableRefreshCount / time);
}

void MonoxideOverlay::sendSiblingFindNodeCall(const TransportAddress& dest)
{
...
void MonoxideOverlay::joinOverlay()
{
...
void MonoxideOverlay::routingInit()
{
    // set join state
    state = INIT;
...
void MonoxideOverlay::routingDeinit()
{
    // delete buckets
...
int MonoxideOverlay::getMaxNumSiblings()
{
    return s;
}

int MonoxideOverlay::getMaxNumRedundantNodes()
{
    return k;
}

int MonoxideOverlay::routingBucketIndex(const OverlayKey& key, bool firstOnLayer)
{
...
MonoxideOverlayBucket* MonoxideOverlay::routingBucket(const OverlayKey& key, bool ensure)
{
    // get bucket index
    ...
    MonoxideOverlayBucket* bucket = routingTable[ num ];
    if (bucket == NULL && ensure)
        bucket = routingTable[ num ] = new MonoxideOverlayBucket( k, comparator );

    // return bucket
    return bucket;
}

bool MonoxideOverlay::routingAdd(const NodeHandle& handle, bool isAlive,
...
//    EV << "[MonoxideOverlay::routingAdd()] @ " << thisNode.getIp()
...
//       << ") ..." << endl;
    ...
    // bucket index
    MonoxideOverlayBucket::iterator i;
    bool result = false;
    ...
    // convert node handle
    MonoxideOverlayBucketEntry kadHandle = handle;
...
//           << "    ... node is already in the sibling table." << endl;
    ...
    MonoxideOverlayBucket* bucket = routingBucket(kadHandle.getKey(), false);
...
//                   << "    ... node not added (node timeout)." << endl;
        ...
        if (siblingTable->isFull()) {
            ...
            MonoxideOverlayBucketEntry oldHandle = siblingTable->back();
...
//           << " which was not yet full." << endl;
        ...
        if (proximityNeighborSelection &&
            ...
            MonoxideOverlayBucket::iterator kickHim, it;
            ...
            if (kickHim->getRtt() > kadHandle.getRtt()) {
                MonoxideOverlayBucketEntry temp = *kickHim;
...
//                   << " (PNS: replacing another node)." << endl;
        ...
        if (enableReplacementCache && (!secureMaintenance || authenticated)) {
            ...
            if (replacementCachePing) {
                MonoxideOverlayBucket::iterator it = bucket->begin();
...
bool MonoxideOverlay::routingTimeout(const OverlayKey& key, bool immediately)
{
    ...
    /* check if node is already in a bucket ---------------------------------*/
    MonoxideOverlayBucket* bucket = routingBucket(key, false);
...
void MonoxideOverlay::refillSiblingTable()
{
    if (siblingTable->size() == 0 ||
        siblingTable->isFull())
        return;
    ...
    if (index < (int)OverlayKey::getLength() &&
        ...
        MonoxideOverlayBucket sortedBucket(k, comparator);
...
void MonoxideOverlay::setBucketUsage(const OverlayKey& key)
{
    MonoxideOverlayBucket* bucket = routingBucket(key, true);
...
bool MonoxideOverlay::isSiblingFor(const NodeHandle& node, const OverlayKey& key,
                            int numSiblings, bool* err)
{
    if (key.isUnspecified())
        error("MonoxideOverlay::isSiblingFor(): key is unspecified!");

    if (state != READY) {
//        EV << "[MonoxideOverlay::isSiblingFor()] @ "
//           << thisNode.getIp()
//           << " (" << thisNode.getKey().toString(16) << ")]\n"
//           << "    state != READY"
//           << endl;
    ...
    if (numSiblings > getMaxNumSiblings()) {
        throw cRuntimeError("MonoxideOverlay::isSiblingFor(): numSiblings too big!");
    }
...
//           << endl;
    ...
    for (MonoxideOverlayBucket::iterator i=siblingTable->begin();
...
void MonoxideOverlay::handleNodeGracefulLeaveNotification()
{
...
bool MonoxideOverlay::handleFailedNode(const TransportAddress& failed)
{
...
// R/MonoxideOverlay
bool MonoxideOverlay::recursiveRoutingHook(const TransportAddress& dest,
                                    BaseRouteMessage* msg)
{
    if (msg->getSrcNode() != thisNode) {
        if (!msg->getDestKey().isUnspecified()) {
            ...
            MonoxideOverlayRoutingInfoMessage* kadRoutingInfoMsg =
                new MonoxideOverlayRoutingInfoMessage();
            ...
            //TODO should be sent after baseroutemsg
                   ...
                   dynamic_cast<MonoxideOverlayRoutingInfoMessage*>(msg->
                           getEncapsulatedPacket())) {
            // alternative mode: infoMsg on its way back
            MonoxideOverlayRoutingInfoMessage* infoMsg =
                static_cast<MonoxideOverlayRoutingInfoMessage*>(msg->decapsulate());
...
NodeVector* MonoxideOverlay::findNode(const OverlayKey& key, int numRedundantNodes,
                               int numSiblings, BaseOverlayMessage* msg)
{
    if (numSiblings > getMaxNumSiblings()) {
        throw cRuntimeError("(MonoxideOverlay::findNode()) numRedundantNodes or numSiblings "
                  "too big!");
    }

#if 0
    if (numRedundantNodes < 2) {
        throw cRuntimeError("MonoxideOverlay::findNode(): For MonoxideOverlay "
    ...
    // R/MonoxideOverlay: in recursive mode just speed up route messages //TODO iterative PR
    ...
    if (mainIndex != -1) {
        MonoxideOverlayBucket* bucket = routingTable[mainIndex];
        if (bucket != NULL && bucket->size()) {
            for (MonoxideOverlayBucket::iterator i=bucket->begin(); i!=bucket->end(); i++) {
                result->add(*i);
                if (returnProxNodes)
                    resultProx->add(*i);
                //EV << "MonoxideOverlay::findNode(): Adding "
    ...
    if (startIndex >= endIndex || !result->isFull()) {
        for (index = startIndex; index >= endIndex; --index) {
            ...
            MonoxideOverlayBucket* bucket = routingTable[index];
            if (bucket != NULL && bucket->size()) {
                for (MonoxideOverlayBucket::iterator i=bucket->begin(); i!=bucket->end(); i++) {
                    result->add(*i);
                    if (returnProxNodes)
                        resultProx->add(*i);//std::make_pair(*i, i->getRtt()));
                    //EV << "MonoxideOverlay::routingGetClosestNodes(): Adding "
        ...
        result->add(thisNode);
        if (returnProxNodes) {
            MonoxideOverlayBucketEntry temp = thisNode;
    ...
    for (index = mainIndex + 1; !result->isFull() && index < numBuckets;
        ...
        if (bucket != NULL && bucket->size()) {
            for (MonoxideOverlayBucket::iterator i=bucket->begin(); i!=bucket->end(); i++) {
                ...
                if (returnProxNodes)
                    resultProx->add(*i);
                //EV << "[MonoxideOverlay::routingGetClosestNodes()]\n"
...
void MonoxideOverlay::handleTimerEvent(cMessage* msg)
{
...
// R/MonoxideOverlay
void MonoxideOverlay::handleUDPMessage(BaseOverlayMessage* msg)
{
    // only used for recursive MonoxideOverlay
    OverlayCtrlInfo* ctrlInfo =
            check_and_cast<OverlayCtrlInfo*>(msg->removeControlInfo());
    MonoxideOverlayRoutingInfoMessage* kadRoutingInfoMsg =
            check_and_cast<MonoxideOverlayRoutingInfoMessage*>(msg);
...
//In MonoxideOverlay this method is used to maintain the routing table.
bool MonoxideOverlay::handleRpcCall(BaseCallMessage* msg)
{
...
//In MonoxideOverlay this method is used to maintain the routing table.
void MonoxideOverlay::handleRpcResponse(BaseResponseMessage* msg,
            ...
            if (state == INIT) {
                ...
                if (newMaintenance) {
                    createLookup()->lookup(getThisNode().getKey(), s, hopCountMax, 0,
                                       new MonoxideOverlayLookupListener(this));
                } else {
                    // schedule bucket refresh timer
                    cancelEvent(bucketRefreshTimer);
                    scheduleAt(simTime(), bucketRefreshTimer);
...
// In MonoxideOverlay this method is used to maintain the routing table.
void MonoxideOverlay::handleRpcTimeout(BaseCallMessage* msg,
...
//        EV << "[MonoxideOverlay:handleRpcTimout() @ " << thisNode.getIp()
//           << " (" << thisNode.getKey().toString(16) << ")]\n"
...
// R/MonoxideOverlay
void MonoxideOverlay::proxCallback(const TransportAddress& node, int rpcId,
...
void MonoxideOverlay::lookupFinished(bool isValid)
{
    if (state == JOIN) {
        cancelEvent(bucketRefreshTimer);
...
void MonoxideOverlay::handleBucketRefreshTimerExpired()
{
    // refresh buckets
    if (state != READY || (((simTime() - siblingTable->getLastUsage()) >
        ...
        if (defaultRoutingType == SEMI_RECURSIVE_ROUTING ||
            ...
            createLookup(EXHAUSTIVE_ITERATIVE_ROUTING)->lookup(
                 ...
                 hopCountMax, 0, new MonoxideOverlayLookupListener(this));
    ...
    if (state == READY) {
        if (siblingTable->size()) {
                    ...
                    if ((routingTable[index] == NULL) ||
                        ...
                        if (defaultRoutingType == SEMI_RECURSIVE_ROUTING ||
                            ...
                            createLookup(EXHAUSTIVE_ITERATIVE_ROUTING)->lookup(
                                ...
                                0, new MonoxideOverlayLookupListener(this));
                            iterativeLookupConfig.redundantNodes = baseRedundantNodes;
                        } else {
            ...
            RECORD_STATS(globalStatistics->recordOutVector(
                                   "MonoxideOverlay: Buckets Refreshed Per Task",
                                   bucketsRefreshedPerTask));
        }
...
OverlayKey MonoxideOverlay::distance(const OverlayKey& x,
                              const OverlayKey& y,
...
void MonoxideOverlay::updateTooltip()
{
    if (hasGUI()) {
...
void MonoxideOverlay::setOwnNodeID()
{
    // node key is set by app
    thisNode.setKey(myKey);
}
